  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.inner}>
        <Typography variant="h1" color={COLORS.primary} style={{ textAlign: 'center' }}>{t('driver.login.title')}</Typography>
        <Typography variant="body" color={COLORS.textMuted} style={{ textAlign: 'center', marginBottom: 40 }}>{t('driver.login.subtitle')}</Typography>
        ...
        <Button title={loading ? t('auth.signingIn') : t('auth.signIn')} onPress={handleLogin} disabled={loading} size="lg" />
      </View>
